...
.status-active{ background-color: rgba(255, 170, 0, 0.10); border-left: 5px solid #ffaa00; }
.status-pending{ background-color: rgba(248, 81, 73, 0.10); border-left: 5px solid #30363d; opacity: 0.6; }
...
tab1, tab2, tab3, tab4 = st.tabs([
    "📖 Introduction", "🚀 Current Assessment", "📈 Progress Graph", "🧬 Biometrics (Beta)"
])
...
with tab1:
    ...
    st.write(
        "Cognitive tracking is a proactive wellness monitor for your brain. By briefly analyzing "
        "subtle patterns, pacing, and vocabulary in your speech over time, CogniTrack monitors "
        "your cognitive vitality and detects early shifts long before they become noticeable."
    )
...
<p>Caused by abnormal protein build-up (amyloid plaques and tau tangles) in the brain.
It typically starts in the hippocampus, primarily affecting learning and short-term memory first.</p>
</div>""", unsafe_allow_html=True)
        try:
            st.image("alzheimers_brain.png", use_container_width=True)
        except:
...
<p>Abnormal balloon-like clumps of protein develop inside nerve cells.
Early signs include severe fluctuations in alertness, visual hallucinations, and Parkinson-like movement issues.</p>
</div>""", unsafe_allow_html=True)
        try:
            st.image("lewy_body_brain.png", use_container_width=True)
        except:
...
<p>Occurs due to restricted blood flow to the brain, often linked to strokes, high blood pressure,
or diabetes. It primarily impacts judgment, planning, and causes slowed thinking.</p>
</div>""", unsafe_allow_html=True)
        try:
            st.image("vascular_brain.png", use_container_width=True)
        except:
...
<p>A leading cause of early-onset decline. It involves progressive nerve loss in the frontal
and temporal lobes, causing drastic changes in personality, empathy, or language abilities.</p>
</div>""", unsafe_allow_html=True)
        try:
            st.image("ftd_brain.png", use_container_width=True)
        except:
...
with tab2:
    ...
    with col_right:
        ...
        if audio:
            # Multi-step spinner so the user sees progress, not a frozen screen
            with st.spinner("⚡ Transcribing audio (this is now much faster)..."):
                try:
                    res = requests.post(
                        "http://127.0.0.1:8000/process",
                        files={"file": (audio.name, audio.getvalue())},
                        timeout=120  # generous timeout for longer recordings
                    )
                    ...
                    st.error("⏱️ Request timed out. Try a shorter audio clip.")
...
with tab3:
    ...
    if len(history) > 0:
        ...
        st.caption("Historical Trend for **Swapnil S.**")
    else:
